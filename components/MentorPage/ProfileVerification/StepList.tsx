...
import { StepListProps } from "../../../lib/types";
import { SmallBulletIcon } from "@/public/assets/icons/svgs";
...
export default function StepList({
  ...
  // Generate unique keys for the progress bars
  const progressBarKeys = ["step1", "step2", "step3", "step4"];
  return (
    <>
      ...
      {step !== 0 && (
        <div className="lg:hidden grid grid-cols-4 grid-rows-1 gap-4 justify-center align-center md:mx-0 mx-5">
          {progressBarKeys.map((key, index) => (
            <div
              key={key}
              className={`h-[8px] border rounded-[16px] ${
                index * 25 < progress
                  ? "bg-Accent1"
                  : step === index + 1
                  ? "border border-Accent1" // Apply the activeProgressBar class for the active step
