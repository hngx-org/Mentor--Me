import Image from "next/image";
import AyraStar from "../../../public/assets/images/landing-page/ayra-star.png";
import Chioma from "../../../public/assets/images/landing-page/chioma.png";
import David from "../../../public/assets/images/landing-page/david.png";
import Oreoluwa from "../../../public/assets/images/landing-page/oreoluwa.png";
import Babafemi from "../../../public/assets/images/landing-page/babafemi.png";

export default function Reviews() {
  return (
    <section className="py-[3rem] my-[6rem] md:my-[9rem]">
      <h3 className="text-[1.5rem] md:text-[2.2rem] font-medium pb-[5rem] text-center">
        What People Say...
      </h3>
      <div className="w-9/10 md:w-9/10 mx-auto pb-[3rem]">
        <div className="top-card-container">
          <div className="w-full md:w-2/5 max-w-[502px] mx-auto md:flex justify-between items-center">
            <figure className="">
              <Image
                src={AyraStar}
                alt=""
                width={100}
                height={100}
                className="w-[80px] mx-auto md:w-[100px]"
              />
            </figure>

            <div className="w-full md:w-4/5 max-w-[384px] mx-auto py-[0.5rem] md:py-[1rem] mt-[1rem] md:mt-0 mb-[5rem] md:mb-0 rounded-[15px] shadow-lg">
              <div className="w-9/10 md:w-5/6 md:max-w-[320px] mx-auto">
                <h3 className="text[#121212] text-[1.2rem] font-medium pb-[0.8rem]">
                  Ayra Star
                </h3>
                <p className="text-[#565656] text-[0.7rem]">
                  `Over the years, Innosons Group has been known for its
                  recruitment of industry professionals and gained so much
                  credibility. Here are Our major areas in the tech and
                  production industry`
                </p>
              </div>
            </div>
          </div>
        </div>

        <div className="md:flex justify-between w-full mx-auto md:py-[6rem]">
          <div className="w-full md:w-2/5 max-w-[502px] mx-auto md:flex justify-between items-center">
            <figure className="">
              <Image
                src={Chioma}
                alt=""
                width={100}
                height={100}
                className="w-[80px] mx-auto md:w-[100px]"
              />
            </figure>

            <div className="w-full md:w-4/5 max-w-[384px] mx-auto py-[0.5rem] md:py-[1rem] mt-[1rem] md:mt-0 mb-[5rem] md:mb-0 rounded-[15px] shadow-lg">
              <div className="w-9/10 md:w-5/6 md:max-w-[320px] mx-auto">
                <h3 className="text[#121212] text-[1.2rem] font-medium pb-[0.8rem]">
                  Chioma Ada-eze
                </h3>
                <p className="text-[#565656] text-[0.7rem]">
                  `Over the years, Innosons Group has been known for its
                  recruitment of industry professionals and gained so much
                  credibility. Here are Our major areas in the tech and
                  production industry`
                </p>
              </div>
            </div>
          </div>

          <div className="w-full md:w-2/5 max-w-[502px] mx-auto md:flex justify-between items-center">
            <figure className="">
              <Image
                src={David}
                alt=""
                width={100}
                height={100}
                className="w-[80px] mx-auto md:w-[100px]"
              />
            </figure>

            <div className="w-full md:w-4/5 max-w-[384px] mx-auto py-[0.5rem] md:py-[1rem] mt-[1rem] md:mt-0 mb-[5rem] md:mb-0 rounded-[15px] shadow-lg">
              <div className="w-9/10 md:w-5/6 md:max-w-[320px] mx-auto">
                <h3 className="text[#121212] text-[1.2rem] font-medium pb-[0.8rem]">
                  David Makinza
                </h3>
                <p className="text-[#565656] text-[0.7rem]">
                  `Over the years, Innosons Group has been known for its
                  recruitment of industry professionals and gained so much
                  credibility. Here are Our major areas in the tech and
                  production industry`
                </p>
              </div>
            </div>
          </div>
        </div>

        <div className="md:flex justify-between w-full md:w-5/6 mx-auto md:pt-[2rem]">
          <div className="w-full md:w-2/5 max-w-[502px] mx-auto md:flex justify-between items-center">
            <figure className="">
              <Image
                src={Oreoluwa}
                alt=""
                width={100}
                height={100}
                className="w-[80px] mx-auto md:w-[100px]"
              />
            </figure>

            <div className="w-full md:w-4/5 max-w-[384px] mx-auto py-[0.5rem] md:py-[1rem] mt-[1rem] md:mt-0 mb-[5rem] md:mb-0 rounded-[15px] shadow-lg">
              <div className="w-9/10 md:w-5/6 md:max-w-[320px] mx-auto">
                <h3 className="text[#121212] text-[1.2rem] font-medium pb-[0.8rem]">
                  Oreoluwa Adedayo
                </h3>
                <p className="text-[#565656] text-[0.7rem]">
                  `Over the years, Innosons Group has been known for its
                  recruitment of industry professionals and gained so much
                  credibility. Here are Our major areas in the tech and
                  production industry`
                </p>
              </div>
            </div>
          </div>

          <div className="w-full md:w-2/5 max-w-[502px] mx-auto md:flex justify-between items-center">
            <figure className="">
              <Image
                src={Babafemi}
                alt=""
                width={100}
                height={100}
                className="w-[80px] mx-auto md:w-[100px]"
              />
            </figure>

            <div className="w-full md:w-4/5 max-w-[384px] mx-auto py-[0.5rem] md:py-[1rem] mt-[1rem] md:mt-0 mb-[5rem] md:mb-0 rounded-[15px] shadow-lg">
              <div className="w-9/10 md:w-5/6 md:max-w-[320px] mx-auto">
                <h3 className="text[#121212] text-[1.2rem] font-medium pb-[0.8rem]">
                  Babafemi Adedayo
                </h3>
                <p className="text-[#565656] text-[0.7rem]">
                  `Over the years, Innosons Group has been known for its
                  recruitment of industry professionals and gained so much
                  credibility. Here are Our major areas in the tech and
                  production industry`
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="w-9/10 md:w-auto mx-auto mt-[4rem] text-center">
        <h3 className="text-[1.2rem] pb-[1.2rem]">
          Click the button to get started in
        </h3>
        <button
          type="button"
          className="bg-[black] text-[white] text-[0.7rem] font-medium w-4/5 md:w-1/6 max-w-[169px] py-[0.8rem] rounded-[8px] outline-none"
        >
          Get Started
        </button>
      </div>
    </section>
  );
}
