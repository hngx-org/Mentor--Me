...
export default function Pagination({
  ...
  // Test
  const testTotalPage = pages.length;
    ...
    <div>
      {loading ? (
        ...
        <div className="flex justify-center items-center mb-20 mt-10 ">
          <div className="flex items-center gap-4 lg:gap-4 rounded-[40px] border-[0.2px] border-Neutra40 px2 py2">
            <button
              type="button"
            ...
            <div className="flex items-center gap3 space-x-3 lg:gap5 lg:space-x-5">
              {pages.map((page) => (
                <button
                  ...
                  key={page}
                  className={
                    currentPage === page ? activeStyle : nonActiveStyle
                  }
            ...
            <button
              type="button"
              onClick={() => onPageChange(currentPage + 1)}
              disabled={currentPage === testTotalPage}
          ...
          <span className="hidden lgflex ml-3">
            Page {currentPage} of {Math.ceil(totalPages / postsPerPage)}
          </span>
        </div>
      )}
    </div>
