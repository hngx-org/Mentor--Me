        ...
        <div className="w-full sm:w-1/2 space-y-7">
            ...
            {tabs.map((item, i) => (
              <Link href={item.link}>
                ...
                >
                  {item.name}
                  {item.reviews && (
                    <span className="font-Hanken text-sm font-bold ml-1 bg-Accent1 rounded-full text-white px-2 py-1">
                      {item.reviews}
                    </span>
                  )}
                </p>
          ...
          {experience.map((exp) => (
            <div key={exp.id} className="flex  justify-between items-center">
              ...
              <p className="font-Hanken text-sm font-normal text-right italic">
                {exp.date}
              </p>
            </div>
          ))}
